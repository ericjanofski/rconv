...
$edit_event_id = ($post->post_parent) ? $post->post_parent : $post->ID;
...
$playlist = get_field('playlist', $event_id);
$playlist_type = get_field('playlist_type', $playlist->ID);
$override_event = get_field('override_event', $event_id) ? ' override-event' : ''; 
 
?>
<div id="tribe-events-event-<?php echo esc_attr( $event_id ); ?>" class="<?php tribe_events_event_classes() ?> <?php echo $playlist_type . $override_event; ?>" data-tribejson='<?php echo esc_attr( tribe_events_template_data( $post ) ); ?>'>
  <span>Event:</span>
	<h3 class="tribe-events-month-event-title"><span class="title"><?php echo $title ?></span>

  <div class="playlist-cont">
    <span>Playlist:</span>
    <?php $playlist = get_field('playlist', $event_id); ?>
    <div class="playlist"><?php echo $playlist->post_title; ?></div>
  </div>
  ...
  <div class="meta">
    <div class="tribe-events-meta-group tribe-events-meta-group-details">
      <dl>
        <?php do_action( 'tribe_events_single_meta_details_section_start' ); ?>
  			<dd>
  				<abbr class="tribe-events-abbr tribe-events-start-date published dtstart" title="<?php esc_attr_e( $start_ts ) ?>"> <?php esc_html_e( $time_formatted ) ?> </abbr>
  			</dd>    
	...
	<div class="edit"><a href="/wp-admin/post.php?post=<?php echo $edit_event_id; ?>&action=edit">Edit</a></div>
	
	</h3></div><!-- #tribe-events-event-# -->